...
second_text_block = layout._blocks[4]
...
cv2.imwrite('cropped/evolution_ratio_depenses_inves.jpg', im[y_1:y_2,x_1:x_2])


model = Pix2StructForConditionalGeneration.from_pretrained("google/deplot")
processor = AutoProcessor.from_pretrained("google/deplot")

image_path = "cropped/evolution_ratio_depenses_inves.jpg"
...
df = pd.DataFrame(table[3:], columns=["Année", "Évolution du ratio des dépenses d'investissement du budget Général par rapport au PIB"])

output='excel'
if not os.path.exists(output):
    os.makedirs(output)

excel_file = "excel/evolution_ratio_depenses_inves.xlsx"
